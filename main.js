...
var LinkMetadataParser = class {
  ...
  getFavicon() {
    return __async(this, null, function* () {
      ...
      for (const selector of selectors) {
        const favicon = (_a = this.htmlDoc.querySelector(selector)) == null ? void 0 : _a.getAttribute("href");
        if (favicon) {
          return yield this.fixImageUrl(favicon, true);
        }
      }
      const { hostname } = new URL(this.url);
  ...
  fixImageUrl(url, isFavicon = false) {
    return __async(this, null, function* () {
      ...
      if (url && url.startsWith("//")) {
        ...
        if (yield checkUrlAccessibility(testUrlHttps, isIcoFile)) {
          image = testUrlHttps;
        } else if (yield checkUrlAccessibility(testUrlHttp, isIcoFile)) {
          image = testUrlHttp;
        }
      } else if (url && url.startsWith("/") && hostname) {
        const testUrlHttps = `https://${hostname}${url}`;
        const testUrlHttp = `http://${hostname}${url}`;
        const resUrlHttps = yield checkUrlAccessibility(testUrlHttps, isIcoFile);
        const resUrlHttp = yield checkUrlAccessibility(testUrlHttp, isIcoFile);
        if (resUrlHttps) {
          image = testUrlHttps;
        } else if (resUrlHttp) {
          image = testUrlHttp;
      ...
      function checkUrlAccessibility(url2, skipImageValidation = false) {
        return __async(this, null, function* () {
...
var CodeBlockGenerator = class {
  ...
  fetchLinkMetadata(url) {
    return __async(this, null, function* () {
      const res = yield (() => __async(null, null, function* () {
        try {
...
var CodeBlockProcessor = class {
  ...
  parseLinkMetadataFromYaml(source) {
    ...
    return {
      ...
      image: yaml.image,
      imageBackground: (_a = yaml.imageBackground) != null ? _a : "#ffffff",
      indent
    };
  }
  genErrorEl(errorMsg) {
...
var ObsidianAutoCardLink = class extends import_obsidian4.Plugin {
  constructor() {
    ...
    this.onPaste = (evt, editor) => __async(this, null, function* () {
      var _a;
      if (!((_a = this.settings) == null ? void 0 : _a.enhanceDefaultPaste)) return;
